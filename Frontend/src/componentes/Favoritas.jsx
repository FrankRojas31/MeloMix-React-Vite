                ...
                if (autenticado) {
                    try {
                        ...
                        const respuesta = await axios.get(
                            `http://localhost:3000/megusta/${decodi64.id}`
                        );
    ...
    const borrar = async (valor) => {
        ...
        if (confirmed) {
          ...
          try {
            const respuesta = await axios.delete(
              `http://localhost:3000/megusta/${Id}`
            );
            if (respuesta.data) {
              setNeedsUpdate(true);
      ...
      const borrarTodo = async () => {
        ...
        if (confirmed) {
            try {
                const respuesta = await axios.delete(
                    `http://localhost:3000/megusta_borrarTodo/${decodi64.id}`
                );
                if (respuesta.data) {
                    setNeedsUpdate(true);
                    Swal.fire('Eliminada correctamente');
    ...
    return (
        <>
            <main className="w-full h-full">
                <h3 className="w-full text-[30px] text-white font-bold text-center relative">Favoritas <i class="nf absolute nf-cod-trash text-[30px] text-[#c00] stroke right-5 cursor-pointer" onClick={() => borrarTodo()}></i></h3>
